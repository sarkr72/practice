...
| `ecr.tf` | The container image registry. `force_delete = true` in non-prod so `terraform destroy` doesn't fail. Lifecycle policy: expire untagged after 7 days, keep last 10 tagged. | The image shelf the deploy workflow pushes to. |
